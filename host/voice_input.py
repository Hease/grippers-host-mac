...
class VoiceResult:
    ...
    # 단어별 신뢰도 [{"w": "퀸을", "p": 0.93}, …]. 낮은 단어만 골라 되묻기
    # (명세 §6 W-401: 70% 미만이면 그 단어만 노란 밑줄 + 후보 시트)에 쓴다.
    # faster-whisper 가 안 주면 빈 리스트다.
        ...
        try:
            if not frames:
                ...
                if len(audio) < SAMPLE_RATE * 0.3:
                    ...
                    # word_timestamps=True 를 줘야 단어별 확률(word.probability)이
                    # 같이 나온다. 없으면 W-401(낮은 신뢰도 되묻기)을 못 만든다.
                    ...
                    result = (VoiceResult(text, words=words) if text
                              else VoiceResult("", error="인식된 말이 없음"))
        except Exception as exc:
